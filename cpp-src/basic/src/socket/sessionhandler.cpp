...
#include "payload/basicbuilder.hpp"
   ...
   bool basic::SessionHandler::cycle() {
      ...
      for (auto& session : this->sessions ) {
         ...
         */
         ...
         auto n = recv(session.fd, raw, sizeof(raw), 0);
   ...
   */
   std::string extramsg = "";
   std::vector<std::string> basic::SessionHandler::splitter(Session& s, const char* originalRaw, int len) {
      std::vector<std::string> results;
      if (originalRaw == NULL || len <= 0) return results;
      std::string newraw(originalRaw, len); // Create a std::string from raw
    
      std::string newraw1 = extramsg + newraw;
      const char* raw = newraw1.c_str();
      len = len + sizeof(raw);
      std::cerr << "---> raw->extra: " << extramsg << len << std::endl;
      ...
      while (pos < len) {
         try {
            ...
            // Validate if tmp is a number
            if (!std::all_of(tmp.begin(), tmp.end(), ::isdigit)) {
                throw std::runtime_error("Invalid format: expected numeric length prefix");
            }
            ...
            if (mlen > len - (pos + 5)) {
               ...
               extramsg = std::string(&ptr[pos],0,len);
               std::cerr << "---> extra msg: " << extramsg << std::endl;
               pos = len;
